...
class SimplGenerator(BaseGenerator):
    ...
    def visit_StringNode(self, node):
        # 1. Start-baken: VVM weet dat er rauwe bytes volgen
        self.emit("STSTR")
        
        # 2. Emit puur en alleen de numerieke ASCII-waarde van elk karakter
        for char in node.value:
            self.emit(ord(char))
            
        # 3. Eind-baken: VVM sluit de string af (en bakt eventueel de 0-terminator)
        self.emit("STREND")
    ...
    def visit_StackStringNode(self, node):
        # 1. Converteer de string naar integer ASCII-waarden en handel escapes af
        vals = []
        i = 0
        s = node.value
        while i < len(s):
            ...
            if char == '\\':
                if i + 1 < len(s):
                    next_char = s[i+1]
                    if next_char == 'n':
                        vals.append(13) # Echte Return (ASCII 13) voor de hash-berekening
                        i += 2
                        continue
                    elif next_char == 't':
                        vals.append(9)  # Tab (ASCII 9)
                        i += 2
                        continue
                    else:
                        # Overige escaped karakters zoals \\ of \"
        ...
        # 2. Bereken de DJB2 Hash exact zoals de host-runtime (STRhash) dat doet
        hash_val = 5381
        for val in vals:
            hash_val = ((hash_val * 33) + val) ## & 0xFFFFFFFFFFFFFFFF  # Masker voor eventuele 64-bit overflow, indien nodig
        
        # 3. Genereer een pure PUSH instructie met de berekende hash-waarde
        # Hierdoor krijgt de VVM tijdens runtime alleen het getal (de hash) te zien!
        self.emit("PUSH")
        self.emit(hash_val)
    ...
    def visit_WordNode(self, node):
        ...
        # 4. VVM Specifics
        if word.upper() in ['OUT', 'SYS', 'HALT', 'FETCH', 'RET', 'RND', 'PRINTS']:
