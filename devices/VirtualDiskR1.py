...
class VirtualDisk:
    ...
    def build_fit16_table(self):
        """
        Scant de real_directory naar bestanden die beginnen met 'sbc_' en eindigen op '.bin'.
        Schrijft de basenames (zonder extensie) naar '_FIT16_.bin', met een maximum van 16 entries.
        Gooit een waarschuwing in de console als er te veel bestanden zijn.
        """
        print("[VirtualDisk R1] Building FIT16 table...")
        
        # Zoek naar alle sbc_*.bin bestanden in de fysieke map
        search_pattern = os.path.join(self.real_directory, "sbc_*.bin")
        matching_files = glob.glob(search_pattern)
        
        # Sorteer ze netjes (optioneel, maar fijn voor voorspelbaarheid in bytecode)
        matching_files.sort()
        ...
        for i, file_path in enumerate(matching_files):
            if i >= limit:
                # Bereken hoeveel bestanden we negeren voor de waarschuwing
                ignored_count = len(matching_files) - limit
                print(f"\u001b[33m[WARNING] VirtualDisk R1: FIT16 limit reached (max {limit})! Ignored {ignored_count} file(s).\u001b[0m")
                break
                
            # Pak de bestandsnaam (bijv. 'sbc_program.bin')
            basename_with_ext = os.path.basename(file_path)
            # Haal de extensie .bin eraf ('sbc_program')
            basename, _ = os.path.splitext(basename_with_ext)
            
            fit_entries.append(basename)
            
        # Bepaal het pad naar het uiteindelijke _FIT16_.bin bestand binnen de virtuele schijf
        fit16_path = os.path.join(self.real_directory, "_FIT16_.bin")
        
        try:
            # Schrijf de basenames weg, één per regel (\n)
            with open(fit16_path, 'w') as f:
                for entry in fit_entries:
                    f.write(entry + "\n")
            print(f"[VirtualDisk R1] FIT16 table successfully written to '{fit16_path}' ({len(fit_entries)} entries).")
        except IOError as e:
            print(f"\u001b[31m[ERROR] VirtualDisk R1: Failed to write _FIT16_.bin: {e}\u001b[0m")
