...
class CPU_M1:
    def __init__(self, memory, interrupt_controller=None, rom_path="bin/stern_rom.json", debug_mode=False):
        ...
        # Instruction Cache
        self.icache_lines = [None] * 64
        self.icache_tags = [-1] * 64
    ...
    def _print_stats(self):
        ...
        cache_hits = self.stats['fetched'] - self.stats['cache_misses']
        print(f"Cache Hits          : {cache_hits}")
        print(f"Cache Misses        : {self.stats['cache_misses']}")
        if self.stats['fetched'] > 0:
            hit_rate = cache_hits / self.stats['fetched'] * 100
    ...
    def _tick_fetch_stage(self):
        ...
        # Optimized Fetch with Direct Mapped Cache
        block_start = current_pc & ~15
        line_index = (block_start >> 4) & 0x3F
        
        if self.icache_tags[line_index] == block_start:
            self.MIR = self.icache_lines[line_index][current_pc & 15]
        else:
            self.stats["cache_misses"] += 1
            block = self.memory.read_block(block_start, 16)
            if block:
                self.icache_lines[line_index] = block
                self.icache_tags[line_index] = block_start
                self.MIR = block[current_pc & 15]
            else:
                # Fallback for boundary conditions (end of memory)
