...
class VirtualNIC:
    ...
    def _process_tx(self):
        ...
        while tx_head != tx_tail:
            ...
            # The driver writes words (16-bit) to the ring.
            # The first HEADER_SIZE words are treated as 8-bit bytes for the SDP header.
            # Subsequent words are treated as 16-bit signed integers for the payload.
            packet_bytes = bytearray()
            for word_idx in range(packet_len): # packet_len is the number of words in RAM
                data_addr = tx_base + current_tail
                word = self.ram.read(data_addr)
                # print(word)  # debug line
                if word_idx < HEADER_SIZE: # First 16 words are header (1 byte per word)
                    packet_bytes.append(word & 0xFF)
                else: # Remaining words are 16-bit signed payload (2 bytes per word)
                    packet_bytes.extend(struct.pack('>h', word)) # '>h' for big-endian signed short
    ...
    def _process_rx_dma(self, data):
        ...
        # Calculate how many words we will write to RAM:
        # 16 words for the header (1 byte per word) 
        # + (remaining bytes / 2) for the 16-bit payload.
        header_bytes = HEADER_SIZE # 16
        payload_bytes = len(data) - header_bytes
        total_words = header_bytes + (payload_bytes // 2)
        required_space = total_words + 1 # +1 for the length word
        ...
        # Truncate if total words exceeds capacity
        if total_words > ring_size - 1:
            print(f"vNIC WARN: Truncating received packet from {total_words} to {ring_size - 1} words.")
            total_words = ring_size - 1

        # 1. Write total words count to the ring at the current head
        self.ram.write(rx_base + rx_head, total_words)
        current_head = (rx_head + 1) % ring_size

        # 2. Write the packet data:
        # Step 2a: Write Header (first 16 bytes as individual words)
        for i in range(header_bytes):
            self.ram.write(rx_base + current_head, data[i])
            current_head = (current_head + 1) % ring_size

        # Step 2b: Write Payload (remaining bytes as 16-bit signed words)
        for i in range(header_bytes, len(data), 2):
            if i + 1 < len(data): # Ensure we have a pair of bytes
                word_val = struct.unpack('>h', data[i:i+2])[0]
                self.ram.write(rx_base + current_head, word_val)
                current_head = (current_head + 1) % ring_size
