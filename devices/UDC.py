...
class UDC:
    def __init__(self, memory, baseadres):
        ...
        self.device_type_instructions[PLOTTER]   = [UDC_DEVICE_NEW, UDC_DEVICE_SEND, UDC_DEVICE_FLIP, UDC_DEVICE_DRAW, UDC_DEVICE_COLOR, UDC_DEVICE_MODE]
    ...
    def tick(self):
        ...
        if command_tuple:
            ...
            if command == UDC_DEVICE_INIT:
                # On INIT, the device becomes initialized and responds with its type.
                # It remains offline until the CPU explicitly brings it online.
            ...
            # Device-specific commands are only handled if the device is online.
            # This now implicitly requires the device to have been initialized.
            if self.online:
                self.handle_command(command, data)
            else:
                # Received a device-specific command while offline.
